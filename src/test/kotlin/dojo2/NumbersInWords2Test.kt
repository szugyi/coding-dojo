...
import dojo2.NumbersInWords2.asWords
import dojo2.NumbersInWords2.getSingleDigitWord
import org.junit.jupiter.api.Test
import kotlin.test.assertEquals


class NumbersInWords2Test {
    ...
    fun `asWords returns the proper word for five digit numbers`() {
        assertEquals("tízezer", 10_000.asWords())
        assertEquals("tizenegyezer", 11_000.asWords())
        assertEquals("húszezer", 20_000.asWords())
        assertEquals("harmincezer", 30_000.asWords())
        assertEquals("negyvenezer", 40_000.asWords())
        assertEquals("ötvenezer", 50_000.asWords())
        assertEquals("hatvanezer", 60_000.asWords())
        assertEquals("hetvenezer", 70_000.asWords())
        assertEquals("nyolcvanezer", 80_000.asWords())
        assertEquals("kilencvenezer", 90_000.asWords())
    }

    @Test
    fun `asWords returns the proper word for six digit numbers`() {
        assertEquals("százezer", 100_000.asWords())
        assertEquals("száztizenegyezer", 111_000.asWords())
        assertEquals("kettőszázezer", 200_000.asWords())
        assertEquals("kilencszázkilencvenkilencezerkilencszázkilencvenkilenc", 999_999.asWords())
    }

    @Test
    fun `asWords returns the proper word for seven digit numbers`() {
        assertEquals("egymilliókettőszázharmincnégyezerötszázhatvanhét", 1_234_567.asWords())
    }
